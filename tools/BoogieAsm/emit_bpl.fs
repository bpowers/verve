...
let rec string_of_exp_r ((r:rel), (side:side)) (e:exp):string =
  let symdiff_nonglobal x = (List.contains x !symdiff_rets) || (List.contains x !symdiff_params) || (List.contains x !symdiff_locals) in
  let symdiff_const x = List.contains x !symdiff_consts in
  let pre loop n x =
    "v" + n + "_u."
  ...
  let post loop n x =
    "v" + n + "_u."
      + ( if symdiff_nonglobal x then
            (if loop then (if List.contains x !symdiff_params then "in_" else "out_") else "") + x
          else if symdiff_const x then x
          else "" + x + "_")
  let is_datatype_field (x:id) = x.Contains("#") && ((x.IndexOf '#') = (x.LastIndexOf '#')) in
  match (r, side, e) with
  | (_, _, EVar x) when List.contains x !symdiff_current_exp_scope -> "(" + x + ")"
...
let rename_var xs x = if List.contains x xs then "#RENAME#" + x else x in
...
let symdiff_clone_call (state:print_state) (p:string):string =
  if not (mem_assoc p state.all_procs) then p else
  let (_, _, _, _, _, specs, _) = assoc p state.all_procs in
  let rels = List.filter (fun (_, spec) ->
    match spec with Requires (IsRel, _) | Ensures (IsRel, _) -> true | _ -> false) specs in
  if rels = [] then p else
  let get_int_ref l =
    if not (mem_assoc p !l) then l := (p, ref 0)::!l;
    assoc p !l in
...
let rec emit_stmt_decl (forall_vars:id list) (state:print_state) (l:loc, s:stmt):unit =
  ...
  let check_assign x = if List.contains x forall_vars then err ("cannot assign to readonly variable " + x) in
  match s with
...
let rec emit_decl (state:print_state) (l:loc, d:decl):unit =
  ...
  | DType (x, _, _, None) ->
    (
      (if not (List.contains x state.all_typedefs) then state.PrintLine ("type " + x + ";"));
        ...
        if not !symdiff then [] else
          ...
          | Procedure _ ->
              ...
              let mods = List.map (fun x ->
                  (if not (mem_assoc x globs) then err ("Found " + x + " in a modifies clause.  Cannot find in global variables."));
                  let (t, lin) = assoc x globs in (x, t, lin)) mods in
              symdiff_proc_mods := (x, mods)::!symdiff_proc_mods;
              mods
          | Implementation _ -> assoc x !symdiff_proc_mods in
      (if !symdiff then
        symdiff_invariant := ("", ([], []));
...
let emit_module (state:print_state) (m:_module):unit =
  ...
  if !symdiff then
    ...
    List.iter (fun (p, count) ->
        let (_, pk, ik, fs, rs, specs, ts) = assoc p state.all_procs in
        for i = 1 to !count do
