...
let sym = ref 0
let gensym () = incr sym; "$$" ^ (sprintf "%d" !sym)
...
let is_32bit_bigint i = BigInteger.Zero <= i && i < (BigInteger.Parse "4294967296")
let is_32bit_bigint_signed i = (BigInteger.Parse "-4294967296") <= i && i < (BigInteger.Parse "4294967296")
...
let operand_of_exp (varMap:varMap) (e:exp):operand =
  ...
  | EVar x when mem_assoc x varMap -> assoc x varMap
...
let rec free_vars_exp (scopeVars:string list) (e:exp):string list =
  match e with
  | EVar x -> if (List.contains x scopeVars) then [] else [x]
...
let check_module (included:_module list) (ifc:_module) (imp:_module) =
  ...
  List.iter (fun x -> if (not (List.contains x mProcDefs)) then err ("module " + ifc.mName + " does not implement procedure " + x)) mProcDecls;
...
//  List.iter (fun x -> if not (List.contains x imp.mImports) then err ("included module " + x + " must be imported")) includeNames;
//  List.iter (fun x -> if not (List.contains x includeNames) then err ("imported module " + x + " must be included")) imp.mImports;
      ...
      | DProc (xp, _, ik, ps, rs, specs, b_opt) ->
        (
          ...
          (
            ...
            let proc_decl = try assoc xp procMap with :? System.Collections.Generic.KeyNotFoundException as x -> err ("procedure " + xp + " not found") in
            ...
            let check_yield e =
              List.iter
                (fun x ->
                  match (List.contains x allVars, List.contains x implVars, List.contains x ifc.mModifies, List.contains x ifc.mYields) with
            ...
            let check_assign forallScope x =
              ...
              | Some fa_xs -> if not (List.contains x fa_xs) then err ("cannot assign to variable " + x + " inside forall statement") else ()
              | None ->
                (
                  if List.contains x local_xs then () else
                  match (List.contains x allVars, List.contains x allRwVars, List.contains x declVars, List.contains x ifc.mModifies, atomicity, List.contains xp ifcProcs) with
                  ...
                  | (true, true, true, false, _, _) -> () // private variable; no one else is allowed to yield on x (REVIEW: require List.contains x implVars here for clarity?)
            ...
            let check_call is_inline ghost forallScope (xc:id) =
              ...
              try
                let ((gc, ac), specs) = assoc xc procMap in
                List.iter (fun (_, s) ->
            ...
            let check_par forallScope xc (xpar, _) =
              try
                let ((_, ac), _) = assoc xc procMap in
                let ((gpar, apar), specs) = assoc xpar procMap in
                List.iter (fun s ->
...
let rec applied_funs_exp (maskVars:string list) (e:exp):string list =
  ...
  | EApply (x, es) ->
      let x = x + "(...)" in
      (if (List.contains x maskVars) then [] else [x]) @ (List.collect f es)
...
let link_check_module (scopeVars:string list) (mIntf:_module, mImpl:_module):string list =
  ...
  let check_decl (_, d:decl):string list * (string * string list) list =
    ...
    | DProc (x, Implementation, _, _, _, _, Some (_, b)) ->
        ([], [(pid x, List.filter (fun y -> not (List.contains y ((pid x)::scopeVars))) (List.map pid (List.collect stmt_calls b)))])
    | DProc (x, Procedure _, _, _, _, _, Some (_, b)) ->
        ([pid x], [(pid x, List.filter (fun y -> not (List.contains y ((pid x)::scopeVars))) (List.map pid (List.collect stmt_calls b)))])
  ...
  List.iter (fun d -> if List.contains d scopeVars then err (d ^ " declared more than once")) decls;
  let defs = pub_defs @ prv_defs in
  let undefs = List.filter (fun d -> not (mem_assoc d defs)) decls in
  let rec check_defs (ds:(string * string list) list):unit =
    ...
    | [] -> ()
    | (d,[])::_ when not (List.contains d decls) -> err (d ^ " not declared in current module")
    | (d,[])::ds when (mem_assoc d ds) -> err (d ^ " defined more than once")
        ...
        (fun ({mName = yM; mYields = yYields}, {mName=_ (* mImports = yImports*) }) ->
          List.iter (fun (_, d) ->
              ...
              | DStaticGhost (x, _, _, _) ->
                (
                  if List.contains x yYields && not (List.contains x xMods) then
                    err ("module " + yM + " yields on non-exported variable " + x)
                )
              | _ -> ())
            xDs;
          List.iter (fun x ->
//              if List.contains x yYields && not (List.contains xM (yM::yImports)) then
...
// grounds that we can't check the import sanity.
              if List.contains x yYields && true then
...
let rec expand_inline_calls (ic:insCtxt) (InlineMap inlines) (l:loc, s:stmt):(insCtxt * loc * stmt) list list =
  ...
  | SInline (_, p, args, _) ->
      ...
      let (inline_inlines, inline_ic, stmts) =
        try assoc p inlines (List.map (operand_of_exp varMap) args)
...
let print_decl (inlines:inlineMap) (_, d:decl): unit =
  match d with
  | DType _ -> ()
  | DFunDecl (x, None, _, Some (EInt i), None, _) when BigInteger.Zero <= i && i < (BigInteger.Parse "4294967296") ->
...
let rec print_modules (ghost_procs:string list) (InlineMap inlines) (p:(_module * _module) list): unit =
    ...
    | DProc (p, pk, inlining, ps, _, _, Some (_, b)) ->
        [(p, fun args ->
          ...
          | Inline ->
              ...
              let stmts = if List.contains p ghost_procs then [] else b in
              ((ctxt, varMap), stmts)
  ...
  | (intf, impl)::ms ->
      ...
      let intf_inlines = List.filter (fun (x, _) -> List.contains x intf_procs) impl_inlines in
...
let main (argv) =
  ...
  (
    ...
    let parse_file comment name =
      ...
      file := name;
      line := 1;
      Parse.start Lex.token (Lexing.LexBuffer<_>.FromTextReader (new StreamReader (name))) in
    if !link then
      let (_, modIfcs, modImps) = List.foldBack (fun (_, x) (b, l1, l2) ->
        (not b, (if b then l1 else x::l1), (if b then x::l2 else l2))) mods (true, [], []) in
      let modIfcImps = List.zip modIfcs modImps in
