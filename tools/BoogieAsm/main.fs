...
let flat_map f x = List.concat (List.map f x)
...
let rec check_exp (scopeVars:string list) (e:exp):string list =
  ...
  | EApply (x, es) ->
      ...
      if (List.contains x scopeVars) then deps else x::deps
...
let rec print_stmt (ctxt:string) (inlines:inlineMap) (s:stmt): unit =
  ...
  | SInline p -> try assoc p inlines () with :? System.Collections.Generic.KeyNotFoundException as x -> err (p ^ " not found")
  | SCall p ->
      assrt (mem_assoc p inlines); (* disallow recursion *)
...
let print_decl (inlines:inlineMap) (d:decl): inlineMap =
  match d with
  | DFunDecl _ -> inlines
  | DFunDef (x, [], EConst i) when BigInteger.Zero <= i && i < (BigInteger.Parse "4294967296") ->
...
let main (argv) =
  try
  (
    let args = List.tail (Microsoft.FSharp.Collections.Array.toList argv) in
    let parse_file name =
